...
function MainPage({ username }) {
  ...
  //
  const [diagramData, setDiagramData] = React.useState<Array<
        Array<object>
    > | null>(null);
  ...
  const handleConvertURI = async () => {
    ...
    const dbURI = (document.getElementById('userURI') as HTMLInputElement)
      .value;
    try {
      // Blur the interaction area
      blurBox?.classList.remove('hidden');
      const response = await axios.post('/submitURI', { dbURI });
  ...
  const GetUsersUris = async (): Promise<void | string> => {
    try {
      ...
      const { data, status } = await axios.get<usersUris>('/uris', {
  ...
  // eslint-disable-next-line react/jsx-filename-extension
    <div id="main-content" className="mainContent">
      <div id="dynamic-about" className="dynamicAbout left-1">
        {showSaveModal && (
          <SaveDatabaseModal
            GetUsersUris={GetUsersUris}
            setShowSaveModal={setShowSaveModal}
          />
        )}
        <h1>How to use radiQL:</h1>
        <div id="circles-container">
          <span id="1" className="circle current-step">
            {instruction > 1 ? (
              <FaCheck style={{ color: 'lime' }} />
            ) : (
              1
            )}
          </span>
          <FaArrowRight />
          <span id="2" className="circle">
            {instruction > 2 ? (
              <FaCheck style={{ color: 'lime' }} />
            ) : (
              '2'
